...
from rssl.utils import align_img
from rssl.transformations import AffineTransform
...
class AffineDeformation3d:
    ...
    def deform_img(self, img, params, interp_mode="bilinear"):
        ...
        return align_img(phi_inv, img, mode=interp_mode), phi_inv
...
def random_affine_augment(
    ...
    img, flow = augmenter(img, params=params, interp_mode="bilinear")
    res = (img,)
    if seg is not None:
...
def random_affine_augment_pair(
    img1, img2, max_random_params=(0.2, 0.2, 3.1416, 0.1), scale_params=1, return_matrix=False, return_flow=False
):
    s, o, a, z = max_random_params
    s *= scale_params
    o *= scale_params
    ...
    img1, flow = augmenter(img1, params=params, interp_mode="bilinear")
    img2, _ = augmenter(img2, params=params, interp_mode="bilinear")
    res = (img1, img2)
    if return_matrix:
        res += (augmenter.build_affine_matrix(len(img1), params),)
    if return_flow:
        res += (flow,)
    return res[0] if len(res) == 1 else res