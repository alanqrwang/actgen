...
def imshow_registration_3d(
    ...
    plot_points = points_m is not None and points_f is not None and points_a is not None
    if plot_points:
...
def imshow_channels(image, save_path=None):
    """
    Plots each channel of an image in separate subplots arranged in a grid layout.

    Parameters:
    image (numpy.ndarray): Input image with shape (H, W, C), where C is the number of channels.
    """
    # Check if the image has at least 3 dimensions (H, W, C)
    if image.ndim != 4:
        raise ValueError("Input image must be a 4D array with shape (C, L, W, H).")

    num_ch, l, w, h = image.shape

    # Determine grid size (rows and columns) to make it as square as possible
    cols = math.ceil(math.sqrt(num_ch))
    ...
    for i in range(num_ch):
        ax = axes[i]
        channel_data = image[i, image.shape[1] // 2, :, :]
        ax.imshow(channel_data)
        ax.axis("off")

    # Hide any unused subplots
    for j in range(num_ch, len(axes)):
        axes[j].axis("off")
